macro_rules ! set_clocks { ($ ($ (# [$ m : meta]) * $ k : ident : $ v : expr ,) *) => { { # [allow (unused)] struct Temp { $ ($ (# [$ m]) * $ k : Option < crate :: time :: Hertz > ,) * } let all = Temp { $ ($ (# [$ m]) * $ k : $ v ,) * } ; crate :: rcc :: set_freqs (crate :: rcc :: Clocks { hclk1 : all . hclk1 . into () , hclk2 : all . hclk2 . into () , hclk3 : all . hclk3 . into () , hse : all . hse . into () , hsi : all . hsi . into () , lse : all . lse . into () , lsi : all . lsi . into () , pclk1 : all . pclk1 . into () , pclk1_tim : all . pclk1_tim . into () , pclk2 : all . pclk2 . into () , pclk2_tim : all . pclk2_tim . into () , pll1_q : all . pll1_q . into () , pll1_r : all . pll1_r . into () , plli2s1_p : all . plli2s1_p . into () , plli2s1_q : all . plli2s1_q . into () , plli2s1_r : all . plli2s1_r . into () , pllsai1_q : all . pllsai1_q . into () , rtc : all . rtc . into () , sys : all . sys . into () , }) ; } } ; }
#[allow(unused)]
...
macro_rules! foreach_interrupt {
    ($($pat:tt => $code:tt;)*) => {
        ...
        __foreach_interrupt_inner!((FMPI2C1,fmpi2c,FMPI2C,ER,FMPI2C1_ER));
        __foreach_interrupt_inner!((FMPI2C1,fmpi2c,FMPI2C,EV,FMPI2C1_EV));
        ...
        __foreach_interrupt_inner!((FMPI2C1_EV));
        __foreach_interrupt_inner!((FMPI2C1_ER));
    };
}
#[allow(unused)]
macro_rules! foreach_peripheral {
    ($($pat:tt => $code:tt;)*) => {
        ...
        __foreach_peripheral_inner!((fmpi2c,FMPI2C1));
